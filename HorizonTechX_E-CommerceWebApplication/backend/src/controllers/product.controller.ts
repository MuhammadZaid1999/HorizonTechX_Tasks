...
export async function getProducts(request: Request, response: Response) {
  ...
  const brandsById = new Map(brands.map((brand) => [brand._id.toString(), brand]));
...
async function getMappedProduct(productId: string) {
  ...
  return mapProduct(
    product,
    new Map(brands.map((brand) => [brand._id.toString(), brand])),
    new Map(categories.map((category) => [category._id.toString(), category])),
  );
}
